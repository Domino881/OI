...
int n,m,k;
int trunk;
pair<int, int> start, last; //pierwszy i ostatni wierzch najdluzszej sciezki
...
bool step(pair<int, int> &v, int &licz){
	if(v.second < trunk){//jesli to lewy
			if(v.first < n-1 && !visited[v.first+1][v.second]){//jesli mozna w dol
				connect(v, mp(v.first+1, v.second), DOWN);
				v = mp(v.first+1, v.second);
				licz--;
				return true;
			}
			else if(v.first > 1 && !visited[v.first-1][v.second]){//jesli mozna w gore
				connect(v, mp(v.first-1, v.second), UP);
				v = mp(v.first-1, v.second);
				licz--;
				return true;
			}
			else if(v.first == 1 || v.first == n-1){//jesli treba w bok
	...
	else if(v.second > trunk){//jesli po prawej
			if(v.first < n-2 && !visited[v.first+1][v.second]){//jesli mozna w dol
				connect(v, mp(v.first+1, v.second), DOWN);
				v = mp(v.first+1, v.second);
				licz--;
				return true;
			}
			else if(v.first > 0 && !visited[v.first-1][v.second]){//jesli mozna w gore
				connect(v, mp(v.first-1, v.second), UP);
				v = mp(v.first-1, v.second);
				licz--;
				return true;
			}
			else if(v.first == 0 || v.first == n-2){//jesli treba w bok
					if(v.second == trunk+1)return false;
